  ...
  return (
    <div className={`${className} relative flex w-full gap-3`}>
      <input
        ...
        onChange={editItem}
        autoFocus
        className="w-full rounded-lg bg-gray-200 bg-opacity-50 py-2 pl-[42px] pr-2 text-xl"
        type="text"
      />
      <span className="absolute left-1.5 top-1/2 -translate-y-1/2">
        ...
        <label
          ...
          htmlFor={data.id}
          className={`${data.isComplete && "-rotate-90"} block h-[30px] w-[30px] cursor-pointer bg-contain bg-no-repeat duration-300`}
        />
      </span>
